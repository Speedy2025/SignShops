...
## COMMANDS
execute if score @e[tag=ss.temp.target,limit=1] ss.interact matches -1 run tellraw @s ["",{"text":"[SignShops] ","color":"green"},{"text":"There are "},{"text":"∞","color":"gold"},{"text":" items in stock."}]
execute if score @e[tag=ss.temp.target,limit=1] ss.interact matches 0.. run tellraw @s ["",{"text":"[SignShops] ","color":"green"},{"text":"There are "},{"score":{"name":"@e[tag=ss.temp.target,limit=1]","objective":"ss.stock"},"color":"gold"},{"text":" item(s) in stock."}]
