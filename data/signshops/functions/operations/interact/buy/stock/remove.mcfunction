...
#Give Items
data modify block 29999984 -5 43376 Items[] set from entity @e[tag=ss.temp.target,limit=1] data.Item
execute if score @e[tag=ss.temp.target,limit=1] ss.stock < @s ss.stock store result block 29999984 -5 43376 Items[0].Count int 1 run scoreboard players get @e[tag=ss.temp.target,limit=1] ss.stock
execute store result score #success ss.matches run loot give @s mine 29999984 -5 43376 air{drop_contents:1b}
execute if score #success ss.matches matches 0 at @s run loot spawn ~ ~ ~ mine 29999984 -5 43376 air{drop_contents:1b}
